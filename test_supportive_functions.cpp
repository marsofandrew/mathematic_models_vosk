...
BOOST_AUTO_TEST_SUITE(check_solveMatrixSweepMethod)
  ...
  BOOST_AUTO_TEST_CASE(invalid_length_a)
  {
    ...
    BOOST_CHECK_THROW(solveMatrixSweepMethod(a, c, b, f), std::exception);
  }
  ...
  BOOST_AUTO_TEST_CASE(solveMatrix)
  {
    ...
    std::vector<double> solution = solveMatrixSweepMethod(a, c, b, f);

    BOOST_CHECK_EQUAL(solution.size(), n);
    BOOST_CHECK_CLOSE(c[0] * solution[0] + b[0] * solution[1], f[0], 10e-12);
    std::cout << f[0] - (c[0] * solution[0] + b[0] * solution[1]) << "\n";
    for (int i = 1; i < n - 1; ++i) {
      BOOST_CHECK_CLOSE(a[i] * solution[i - 1] + c[i] * solution[i] + b[i] * solution[i + 1], f[i], 10e-12);
      std::cout << f[i] - (a[i] * solution[i - 1] + c[i] * solution[i] + b[i] * solution[i + 1]) << "\n";
    }
    BOOST_CHECK_CLOSE(c[3] * solution[3] + a[3] * solution[2], f[3], 10e-12);
